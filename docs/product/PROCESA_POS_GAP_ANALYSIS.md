...
| **Gestión de Caja y Turnos** | **100% COMPLETADO (ETAPA 7C)** (`src/app/app/pos/cash-sessions/`) | Completo en `SaaS Minimarket` | Incorporado: **Arqueo Ciego** obligatorio sin exposición previa del saldo teórico, desglosador por denominaciones, conciliación de turno (Cuadrada/Sobrante/Faltante) y comprobante térmico imprimible de cierre. | Mantener y validar en QA comercial. | **COMPLETADO (7C)** |
| **Kardex Valorizado y Costo Promedio** | **100% COMPLETADO (ETAPA 7D)** (`supabase/migrations/071_...sql`) | Completo en `SaaS Ventas e Inv.` | Incorporado: Cálculo formal de **Costo Promedio Ponderado Móvil (CPP)** con 4 decimales bajo bloqueo pesimista en PostgreSQL, consolidación de stock multi-almacén, trazabilidad Kardex `PURCHASE_IN` y comparador visual en formulario de compras. | Mantener y validar en QA comercial. | **COMPLETADO (7D)** |
| **Módulo de Compras a Proveedores** | **100% COMPLETADO (ETAPA 7D)** (`src/app/app/pos/purchases/`) | Completo en `SaaS Minimarket` | Formulario transaccional con desglose de ítems, precio unitario de costo con 4 decimales, validación de RUC y generación correlativa `COM-XXXXXX`. | Mantener y validar en QA comercial. | **COMPLETADO (7D)** |
