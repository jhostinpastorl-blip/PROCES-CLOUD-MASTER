  ...
  {await Promise.all(companies.map(async c=>{
   ...
   return <section className="tenant-section" key={c.companyId}>
    ...
    <section className="table-card"><div className="table-card-head"><div><h3>Invitaciones</h3><p>Últimas invitaciones enviadas.</p></div></div>
     {invites?.length?<div className="premium-table invite-table"><div className="premium-tr head"><span>Correo</span><span>Vencimiento</span><span>Estado</span></div>{invites.map(i=><div className="premium-tr" key={i.id}><span><b>{i.email}</b><small>Acceso pendiente</small></span><span>{new Date(i.expires_at).toLocaleString("es-PE")}</span><span><StatusChip tone={i.status==="pending"?"warning":"success"}>{i.status}</StatusChip></span></div>)}</div>:<EmptyState title="Sin invitaciones" text="Las invitaciones enviadas aparecerán aquí."/>}
    </section>
   </section>
  }))}
 </main>
