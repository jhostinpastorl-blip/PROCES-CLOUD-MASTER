  ...
  {await Promise.all(companies.map(async c=>{
   ...
   return <section className="tenant-section" key={c.companyId}>
    ...
    {catalog?.length?<div className="module-market-grid real-module-market">{catalog.map(m=>{
      ...
      return <article key={m.id}><div className="module-market-head"><span>{m.code}</span><StatusChip tone={active?"success":m.status!=="disabled"?"info":"neutral"}>{active?"Activo":m.status!=="disabled"?"Disponible":"No disponible"}</StatusChip></div><h3>{m.name}</h3><p>{"Módulo empresarial integrado a PROCESA Cloud."}</p><div className="module-market-foot"><button className={active?"secondary-btn":"primary-btn"} disabled={!canManage||m.status==="disabled"}>{active?"Configurar":"Activar módulo"}</button></div></article>
